...
const apiURL = (path) => new URL(path, location.origin + APP_BASE).toString();
...
const sendForgot = async () => {
  ...
  try {
    ...
    // 呼叫你的 forgot_password2.php
    const res = await fetch('forgot_password2.php', {
      method: 'POST',
      headers: { 'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8' },
      body: new URLSearchParams({ account: forgotAccount.value })
    });

    if (!res.ok) throw new Error(`服務錯誤：${res.status}`);

    const text = await res.text();

    if (text.trim() === 'ok') {
      modalForgot?.hide();
      notify('已送出', '請至註冊信箱查收，或聯絡系統管理員協助重設。', 'success');
    } else {
      setError(text);  // 會顯示「找不到此帳號」或「寄信失敗...」
    }
  } catch (e) {
    ...
    const loginSubmit = async () => {
      ...
      loading.value = true;
      try {
        const res = await fetch(apiURL('user.php?do=login_sub'), {
          method: 'POST',
          headers: { 'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8' },
        ...
        if (!res.ok) {
          // 明確顯示 HTTP 錯誤（像你現在的 404）
          throw new Error(`登入服務錯誤：${res.status} (${res.statusText})`);
        }

        // 後端請回標準 JSON：{ ok:true|false, msg:'...', needRole:false }
